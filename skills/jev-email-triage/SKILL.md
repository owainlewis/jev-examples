...
Work from the jev-examples repository root, where `requirements.txt` and `jev_tutorial/` exist. Require a named input file. Each record must have nonempty string `id`, `subject`, and `body` fields with a unique ID. Start with `data/emails.json` for the tutorial.
...
3. Run `.venv/bin/python -m jev_tutorial.email_cli data/emails.json` from the repository root, replacing the final path only with the user's selected file. Use proper shell quoting for paths.
