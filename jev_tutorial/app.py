...
def create_app(database=None, classifier=None):
        ...
        try:
            ...
            with connect() as connection:
                connection.execute(
                    ...
                    "ELSE ? END, result = ?, error = NULL WHERE id = ?",
    ...
    def retry_ticket(ticket_id):
        ticket = get_ticket(ticket_id)
        if ticket["status"] not in ("failed", "pending"):
            abort(409, "This ticket already has a classification.")
        evaluate(ticket_id)
        return redirect(url_for("ticket_detail", ticket_id=ticket_id), code=303)
